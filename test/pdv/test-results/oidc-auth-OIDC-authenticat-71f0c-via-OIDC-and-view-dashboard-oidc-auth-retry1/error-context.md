...
- Name: oidc-auth.spec.ts >> OIDC authentication flow >> pdv-test user can login via OIDC and view dashboard
- Location: tests/oidc-auth.spec.ts:33:7
...
> 35  |     await page.goto(`${GRAFANA_URL}/login`, { ignoreHTTPSErrors: true });
  ...
  130 |       });
  131 |       if (!loginRes.ok) { test.skip(); return; }
  132 |       const { auth } = await loginRes.json();
  133 | 
  134 |       // Try to read a secret — should be denied
  135 |       const secretRes = await fetch(`${AUTH_URL}/v1/convocate/data/influxdb`, {
```