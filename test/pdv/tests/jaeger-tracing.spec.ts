/**
 * Jaeger Distributed Tracing — Post-Deployment Verification Tests
 *
 * Validates that Jaeger tracing is operational: query API is accessible,
...
test.describe("Jaeger query API availability", () => {
  test("Jaeger datasource is configured in Grafana", async () => {
    try {
      ...
      // Jaeger datasource should exist
  ...
  test("Jaeger query API is reachable via Grafana proxy", async () => {
    try {
      ...
      // Query Jaeger services via Grafana datasource proxy
      const res = await fetch(
        `${GRAFANA_URL}/api/datasources/proxy/${jaeger.id}/api/services`,
        { headers: grafanaAuth() }
      );
      // Accept 200 (working), 502 (Jaeger unreachable), or 404 (proxy not supported)
...
test.describe("Jaeger service registration", () => {
  test("convocate-api service appears in Jaeger service list", async () => {
    try {
      const dsRes = await fetch(`${GRAFANA_URL}/api/datasources`, {
        headers: grafanaAuth(),
      });
      ...
      expect(hasConvocate, "convocate-api service should appear in Jaeger").toBe(true);
    } catch {
      test.skip();
    }
  });
...
test.describe("Jaeger trace generation", () => {
